...
class ArgumentData:
    ...
    parameter_scan_data: ParameterScanData = Field(
        description="parameter scan data", default=None
    )
...
def run_parameter_scan(
    ...
    df_all_parameters = basico.model_info.get_parameters(
    ...
    if (
        ...
        not in all_parameters + all_species
    ):
        logger.error(
    ...
    for species_name in arg_data.parameter_scan_data.species_names:
        ...
        for param_value in arg_data.parameter_scan_data.species_parameter_values:
            ...
            col_name = (
                f"{arg_data.parameter_scan_data.species_parameter_name}_{param_value}"
            )
            df_param_scan[col_name] = model_object_copy.simulation_results[species_name]

        logger.log(
            logging.INFO, "Parameter scan results with shape %s", df_param_scan.shape
        )
...
class ParameterScanTool(BaseTool):
    ...
    def _run(
        ...
        """
        logger.log(
            logging.INFO, "Calling parameter_scan tool %s, %s", sys_bio_model, arg_data
        )
        sbml_file_path = (
