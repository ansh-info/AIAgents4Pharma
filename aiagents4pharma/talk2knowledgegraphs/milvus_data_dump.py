...
class SystemDetector:
    ...
    def install_packages(self):
        ...
        logger.info("Installing packages for %s system%s", 
                   self.os_type, " with GPU support" if self.use_gpu else "")
...
class DynamicDataLoader:
    ...
    def _import_libraries(self):
        ...
        from pymilvus import (
            db, connections, FieldSchema, CollectionSchema, 
        ...
        self.pymilvus_modules = {
            'db': db, 'connections': connections, 'FieldSchema': FieldSchema,
            'CollectionSchema': CollectionSchema, 'DataType': DataType, 
            'Collection': Collection, 'utility': utility
        }
        ...
        if self.detector.use_gpu:
            try:
                ...
                logger.error("[DATA LOADER] cudf or cupy not found. Please ensure they are installed correctly.")
                ...
                if not os.getenv('FORCE_CPU', 'false').lower() == 'true':
                    logger.error("GPU libraries required but not available. Set FORCE_CPU=true to use CPU mode.")
                    sys.exit(1)
                else:
    ...
    def _read_dataframe(self, file_path: str, columns: Optional[List[str]] = None) -> Union['pd.DataFrame', 'cudf.DataFrame']:
    ...
    def _concat_dataframes(self, df_list: List, ignore_index: bool = True) -> Union['pd.DataFrame', 'cudf.DataFrame']:
    ...
    def create_nodes_collection(self, nodes_df):
        ...
        node_fields = [
            self.pymilvus_modules['FieldSchema'](name="node_index",
                        dtype=self.pymilvus_modules['DataType'].INT64,
                        is_primary=True),
            self.pymilvus_modules['FieldSchema'](name="node_id",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=1024),
            self.pymilvus_modules['FieldSchema'](name="node_name",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=1024,
                        enable_analyzer=True,
                        enable_match=True),
            self.pymilvus_modules['FieldSchema'](name="node_type",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=1024,
                        enable_analyzer=True,
                        enable_match=True),
            self.pymilvus_modules['FieldSchema'](name="desc",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=40960,
                        enable_analyzer=True,
                        enable_match=True),
            self.pymilvus_modules['FieldSchema'](name="desc_emb",
        ...
        collection.create_index(field_name="desc_emb",
                                index_params={"index_type": self.vector_index_type,
    ...
    def create_node_type_collections(self, nodes_df):
        ...
        for node_type, nodes_df_ in self.tqdm(nodes_df.groupby('node_type'),
            ...
            node_fields = [
                ...
                self.pymilvus_modules['FieldSchema'](name="desc_emb",
                            dtype=self.pymilvus_modules['DataType'].FLOAT_VECTOR,
                            dim=desc_dim),
                self.pymilvus_modules['FieldSchema'](name="feat",
                            dtype=self.pymilvus_modules['DataType'].VARCHAR,
                            max_length=40960,
                            enable_analyzer=True,
                            enable_match=True),
                self.pymilvus_modules['FieldSchema'](name="feat_emb",
                            dtype=self.pymilvus_modules['DataType'].FLOAT_VECTOR,
                            dim=feat_dim),
            ]
    ...
    def create_edges_collection(self, edges_enrichment_df, edges_embedding_df: List):
        ...
        if self.use_gpu:
            ...
            emb_dim = len(first_edge_emb) if isinstance(first_edge_emb, list) else len(first_edge_emb.tolist())
        
        edge_fields = [
            self.pymilvus_modules['FieldSchema'](name="triplet_index",
                        dtype=self.pymilvus_modules['DataType'].INT64,
                        is_primary=True,
                        auto_id=False),
            self.pymilvus_modules['FieldSchema'](name="head_id",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=1024),
            self.pymilvus_modules['FieldSchema'](name="head_index",
                        dtype=self.pymilvus_modules['DataType'].INT64),
            self.pymilvus_modules['FieldSchema'](name="tail_id",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=1024),
            self.pymilvus_modules['FieldSchema'](name="tail_index",
                        dtype=self.pymilvus_modules['DataType'].INT64),
            self.pymilvus_modules['FieldSchema'](name="edge_type",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=1024),
            self.pymilvus_modules['FieldSchema'](name="display_relation",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
                        max_length=1024),
            self.pymilvus_modules['FieldSchema'](name="feat",
                        dtype=self.pymilvus_modules['DataType'].VARCHAR,
        ...
        if not self.pymilvus_modules['utility'].has_collection(edge_coll_name):
            collection = self.pymilvus_modules['Collection'](name=edge_coll_name, schema=edge_schema)
        else:
        ...
        for edges_df in self.tqdm(edges_embedding_df, desc="Processing edge chunks"):
            ...
            if self.use_gpu:
                edge_emb_cp = self.cp.asarray(merged_edges_df["edge_emb"].list.leaves).astype(self.cp.float32).reshape(merged_edges_df.shape[0], -1)
