...
def test_load_and_split_pdf_success(mock_splitter_cls, mock_loader_cls, base_args):
    ...
    chunks = load_and_split_pdf(**base_args)

    assert len(chunks) == 1
    assert "P123_0" in base_args["documents_dict"]
    stored_chunk = base_args["documents_dict"]["P123_0"]
    ...
    assert stored_chunk.metadata["source"] == base_args["pdf_url"]
